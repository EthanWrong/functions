...
def ticket_sales(notices):
    ...
    while want_to_sell != "n":
        ...
        if want_to_sell == "y":
            number_of_tickets_to_buy = int(input("How many tickets do you want to buy?"))
            ticket_to_buy = input(
                "What type of ticket do you want? \nA for adult, or \nS for student, or \nC for child, or \nG for gift voucher, or \nAnything else to cancel \n>> ").lower()
            if ticket_to_buy == "a":
                a_sales += number_of_tickets_to_buy
            elif ticket_to_buy == "s":
                s_sales += number_of_tickets_to_buy
            elif ticket_to_buy == "c":
                c_sales += number_of_tickets_to_buy
            elif ticket_to_buy == "g":
                g_sales += number_of_tickets_to_buy
            else:
...
ticket_sales("Out of buttered popcorn till Wednesday.")
